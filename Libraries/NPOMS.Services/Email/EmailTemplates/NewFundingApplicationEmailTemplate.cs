...
{
	public class NewFundingApplicationEmailTemplate : IEmailTemplate
	{
		private FundingApplication _fundingApplication;

		public NewFundingApplicationEmailTemplate Init(FundingApplication fundingApplication)
		{
		...
		public async Task SubmitToQueue()
		{
			...
			var logger = EngineContext.Current.Resolve<ILogger<NewFundingApplicationEmailTemplate>>();
			...
			var emailTemplate = await emailTemplateService.GetByType(EmailTemplateTypeEnum.NewFundingApplication);
			...
			try
			{
				...
				users.Add(await userRepository.GetUserByIdAsync(application.CreatedUserId));

				if (application.UpdatedUserId != null && application.CreatedUserId != application.UpdatedUserId)
					users.Add(await userRepository.GetUserByIdAsync(Convert.ToInt32(application.UpdatedUserId)));
				...
				foreach (var user in users)
				{
					EmailQueue emailQueue = new EmailQueue()
					{
						...
						Message = ReplacePlaceholders(emailTemplate.Body, application, user, requestOrigin),
						Subject = ReplacePlaceholders(emailTemplate.Subject, application, user, requestOrigin),
			...
			{
				logger.LogError($"Something went wrong inside NewFundingApplicationEmailTemplate-SubmitToQueue action: {ex.Message} Inner Exception: {ex.InnerException}");
				throw;
			}
		}
