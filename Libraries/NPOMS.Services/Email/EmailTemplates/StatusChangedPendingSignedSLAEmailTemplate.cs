...
{
	public class StatusChangedPendingSignedSLAEmailTemplate : IEmailTemplate
	{
		private Application _application;

		public StatusChangedPendingSignedSLAEmailTemplate Init(Application application)
		{
		...
		public async Task SubmitToQueue()
		{
			...
			var logger = EngineContext.Current.Resolve<ILogger<StatusChangedPendingSignedSLAEmailTemplate>>();
			...
			var emailTemplate = await emailTemplateService.GetByType(EmailTemplateTypeEnum.StatusChangedPendingSignedSLA);
			...
			{
				logger.LogError($"Something went wrong inside StatusChangedPendingSignedSLAEmailTemplate-SubmitToQueue action: {ex.Message} Inner Exception: { ex.InnerException}");
				throw;
			}
		}
