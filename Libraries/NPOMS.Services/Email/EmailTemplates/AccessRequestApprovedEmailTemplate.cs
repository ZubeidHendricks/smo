...
{
	public class AccessRequestApprovedEmailTemplate : IEmailTemplate
	{
		private UserNpo _userNpoMapping;

		public AccessRequestApprovedEmailTemplate Init(UserNpo userNpoMapping)
		{
		...
		public async Task SubmitToQueue()
		{
			...
			var logger = EngineContext.Current.Resolve<ILogger<AccessRequestApprovedEmailTemplate>>();
			...
			var emailTemplate = await emailTemplateService.GetByType(EmailTemplateTypeEnum.AccessRequestApproved);
			...
			{
				logger.LogError($"Something went wrong inside AccessRequestApprovedEmailTemplate-SubmitToQueue action: {ex.Message} Inner Exception: { ex.InnerException}");
				throw;
			}
		}
