...
{
	public class RejectedOrganisationApprovalEmailTemplate : IEmailTemplate
	{
		private Npo _npo;

		public RejectedOrganisationApprovalEmailTemplate Init(Npo npo)
		{
			this._npo = npo;
			return this;
		}
		...
		public async Task SubmitToQueue()
		{
			...
			var logger = EngineContext.Current.Resolve<ILogger<RejectedOrganisationApprovalEmailTemplate>>();
			...
			var emailTemplate = await emailTemplateService.GetByType(EmailTemplateTypeEnum.RejectedOrganisationApproval);
			...
			{
				logger.LogError($"Something went wrong inside RejectedOrganisationApprovalEmailTemplate-SubmitToQueue action: {ex.Message} Inner Exception: {ex.InnerException}");
				throw;
			}
		}
