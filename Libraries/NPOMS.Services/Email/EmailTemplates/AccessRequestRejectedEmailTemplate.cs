...
{
	public class AccessRequestRejectedEmailTemplate : IEmailTemplate
	{
		private UserNpo _userNpoMapping;

		public AccessRequestRejectedEmailTemplate Init(UserNpo userNpoMapping)
		{
		...
		public async Task SubmitToQueue()
		{
			...
			var logger = EngineContext.Current.Resolve<ILogger<AccessRequestRejectedEmailTemplate>>();
			...
			var emailTemplate = await emailTemplateService.GetByType(EmailTemplateTypeEnum.AccessRequestRejected);
			...
			{
				logger.LogError($"Something went wrong inside AccessRequestRejectedEmailTemplate-SubmitToQueue action: {ex.Message} Inner Exception: { ex.InnerException}");
				throw;
			}
		}
