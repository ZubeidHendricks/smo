...
{
	public class StatusChangedPendingReviewerSatisfactionEmailTemplate : IEmailTemplate
	{
		private Application _application;

		public StatusChangedPendingReviewerSatisfactionEmailTemplate Init(Application application)
		{
		...
		public async Task SubmitToQueue()
		{
			...
			var logger = EngineContext.Current.Resolve<ILogger<StatusChangedPendingReviewerSatisfactionEmailTemplate>>();
			...
			var emailTemplate = await emailTemplateService.GetByType(EmailTemplateTypeEnum.StatusChangedPendingReviewerSatisfaction);
			...
			{
				logger.LogError($"Something went wrong inside StatusChangedPendingReviewerSatisfactionEmailTemplate-SubmitToQueue action: {ex.Message} Inner Exception: {ex.InnerException}");
				throw;
			}
		}
