...
{
	public class WorkplanActualPendingApprovalEmailTemplate : IEmailTemplate
	{
		private WorkplanActual _workplanActual;
		private Application _application;

		public WorkplanActualPendingApprovalEmailTemplate Init(WorkplanActual workplanActual, Application application)
		{
		...
		public async Task SubmitToQueue()
		{
			...
			var logger = EngineContext.Current.Resolve<ILogger<WorkplanActualPendingApprovalEmailTemplate>>();
			...
			var emailTemplate = await emailTemplateService.GetByType(EmailTemplateTypeEnum.WorkplanActualPendingApproval);
			...
			var rolePermissions = await rolePermissionRepository.GetByPermissionId((int)PermissionEnum.ApproveWorkplanActual);
			...
			{
				logger.LogError($"Something went wrong inside WorkplanActualPendingApprovalEmailTemplate-SubmitToQueue action: {ex.Message} Inner Exception: {ex.InnerException}");
				throw;
			}
		}
