...
{
	public class StatusChangedOrgCommentsEmailTemplate : IEmailTemplate
	{
		private Application _application;

		public StatusChangedOrgCommentsEmailTemplate Init(Application application)
		{
		...
		public async Task SubmitToQueue()
		{
			...
			var logger = EngineContext.Current.Resolve<ILogger<StatusChangedOrgCommentsEmailTemplate>>();
			...
			var emailTemplate = await emailTemplateService.GetByType(EmailTemplateTypeEnum.StatusChangedOrgComments);
			...
			{
				logger.LogError($"Something went wrong inside StatusChangedOrgCommentsEmailTemplate-SubmitToQueue action: {ex.Message} Inner Exception: { ex.InnerException}");
				throw;
			}
		}
