...
{
    public class FundingStatusChangedPendingEmailTemplate : IEmailTemplate
    {
        private FundingCaptureViewModel _fundingCapture;

        public FundingStatusChangedPendingEmailTemplate Init(FundingCaptureViewModel fundingCapture)
        {
        ...
        public async Task SubmitToQueue()
        {
            ...
            var logger = EngineContext.Current.Resolve<ILogger<FundingStatusChangedPendingEmailTemplate>>();
            ...
            var emailTemplate = await emailTemplateService.GetByType(EmailTemplateTypeEnum.FundingStatusChangedPending);
            ...
            try
            {
                ...
                switch (fundingStatus)
                {
                    case StatusEnum.PendingApproval:
                        action = "approve";
                        users = await userRepository.GetUsersByPermissionId((int)PermissionEnum.ApproveFundingCapture);
                        break;
                }
            ...
            {
                logger.LogError($"Something went wrong inside FundingStatusChangedPendingEmailTemplate-SubmitToQueue action: {ex.Message} Inner Exception: {ex.InnerException}");
                throw;
            }
        }
