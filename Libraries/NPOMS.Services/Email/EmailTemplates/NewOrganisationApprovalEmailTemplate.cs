...
{
	public class NewOrganisationApprovalEmailTemplate : IEmailTemplate
	{
		private Npo _npo;

		public NewOrganisationApprovalEmailTemplate Init(Npo npo)
		{
			this._npo = npo;
			return this;
		}
		...
		public async Task SubmitToQueue()
		{
			...
			var logger = EngineContext.Current.Resolve<ILogger<NewOrganisationApprovalEmailTemplate>>();
			...
			var emailTemplate = await emailTemplateService.GetByType(EmailTemplateTypeEnum.NewOrganisationApproval);
			...
			{
				logger.LogError($"Something went wrong inside NewOrganisationApprovalEmailTemplate-SubmitToQueue action: {ex.Message} Inner Exception: {ex.InnerException}");
				throw;
			}
		}
