...
{
	public class StatusChangedAcceptedSLAEmailTemplate : IEmailTemplate
	{
		private Application _application;

		public StatusChangedAcceptedSLAEmailTemplate Init(Application application)
		{
		...
		public async Task SubmitToQueue()
		{
			...
			var logger = EngineContext.Current.Resolve<ILogger<StatusChangedAcceptedSLAEmailTemplate>>();
			...
			var emailTemplate = await emailTemplateService.GetByType(EmailTemplateTypeEnum.StatusChangedAcceptedSLA);
			...
			var adminUsers = await userRepository.GetByIds((int)RoleEnum.Admin, application.ApplicationPeriod.DepartmentId);
			...
			{
				logger.LogError($"Something went wrong inside StatusChangedAcceptedSLAEmailTemplate-SubmitToQueue action: {ex.Message} Inner Exception: { ex.InnerException}");
				throw;
			}
		}
