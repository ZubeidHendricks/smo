...
{
	public class StatusChangedRejectedEmailTemplate : IEmailTemplate
	{
		private Application _application;

		public StatusChangedRejectedEmailTemplate Init(Application application)
		{
		...
		public async Task SubmitToQueue()
		{
			...
			var logger = EngineContext.Current.Resolve<ILogger<StatusChangedRejectedEmailTemplate>>();
			...
			var emailTemplate = await emailTemplateService.GetByType(EmailTemplateTypeEnum.StatusChangedRejected);
			...
			{
				logger.LogError($"Something went wrong inside StatusChangedRejectedEmailTemplate-SubmitToQueue action: {ex.Message} Inner Exception: { ex.InnerException}");
				throw;
			}
		}
