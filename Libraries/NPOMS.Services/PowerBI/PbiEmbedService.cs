...
namespace NPOMS.Services.PowerBI
{
	...
	public class PbiEmbedService
	{
		...
		private readonly IEmbeddedReportService _embeddedReportService;
		private readonly string urlPowerBiServiceApiRoot = "https://api.powerbi.com";

		public PbiEmbedService(AadService aadService, IEmbeddedReportService embeddedReportService)
		{
			this.aadService = aadService;
			this._embeddedReportService = embeddedReportService;
		}
		...
		public EmbedParams GetEmbedParams(int id, [Optional] Guid additionalDatasetId)
		{
			var report = this._embeddedReportService.GetEmbeddedReportById(id).Result;

			var workspaceId = new Guid(report.WorkspaceId);
			var reportId = new Guid(report.ReportId);
		...
		public EmbedToken GetEmbedToken(Guid reportId, IList<Guid> datasetIds, [Optional] Guid targetWorkspaceId)
		{
			...
			List<EffectiveIdentity> rlsList = null;

			//var rlsList = new List<EffectiveIdentity>();
			//var rls = new EffectiveIdentity(username: "gabriel.leroux@westerncape.gov.za", new List<string>() { datasetIds[0].ToString() });
