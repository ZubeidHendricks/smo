...
@Component({
  selector: 'app-targets-businessplan',
  templateUrl: './targets-businessplan.component.html',
  styleUrls: ['./targets-businessplan.component.css'],
  providers: [MessageService, ConfirmationService]
})
export class TargetsBusinessPlanComponent implements OnInit {
  ...
  private loadFrequencies() {
    ...
    this._dropdownRepo.getEntities(DropdownTypeEnum.Frequencies, false).subscribe(
      (results) => {
        this.frequencies = results.filter(x => x.id === FrequencyEnum.Quarterly);
        this._spinner.hide();
      },
      (err) => {
        this._loggerService.logException(err);
  ...
  private buildMenu() {
    this.menuActions = [
      ...
      },
      {
        ...
        command: () => {
          this._router.navigateByUrl('businessplan-indicator/manage/' + this.application.npoId);
        }
      }
    ];
  }
  ...
  private saveItems() {
    if (this.canContinue()) {
      ...
      let sumOfMonthlyTargets = (this.selectedWorkplanTarget.mar + this.selectedWorkplanTarget.jun + this.selectedWorkplanTarget.sep + this.selectedWorkplanTarget.dec);
      ...
      this._indicatorRepo.manageTarget(this.selectedWorkplanTarget).subscribe(
        (resp) => {
          ...
          if (this.activity.target != sumOfMonthlyTargets)
            this._messageService.add({ severity: 'warn', summary: 'Warning', detail: 'Sum of Quarterly Targets not equal to Activity Target.' });
