    ...
    this._npoProfile.getProgrammeDeliveryDetails(Number(this.selectedApplicationId)).subscribe(
      (results) => {
        if (results != null) {
          ...
          this.selectedProgramDeliveryDetails = results.filter(deliveryDetail => deliveryDetail.isActive && deliveryDetail.programId === this.programId && deliveryDetail.subProgrammeId === this.subProgramId && deliveryDetail.isSelected === true);
        } 
        this._spinner.hide();
      },
      (err) => {
