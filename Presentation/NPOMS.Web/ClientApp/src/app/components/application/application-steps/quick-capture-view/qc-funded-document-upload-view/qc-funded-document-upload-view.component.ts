...
import { IFundingApplicationDetails, IApplication, IUser, IDocumentStore, IDocumentType, IProjectInformation, IMonitoringAndEvaluation, IMyContentLink, INpoProfile, IApplicationPeriod } from 'src/app/models/interfaces';
...
@Component({
  selector: 'app-qc-funded-document-upload-view',
  templateUrl: './qc-funded-document-upload-view.component.html',
  styleUrls: ['./qc-funded-document-upload-view.component.css']
})
export class QcFundedDocumentUploadViewComponent implements OnInit {
 ...
 // filteredDocuments: IMyContentLink[];
  ...
  private getDocuments() {
    this._documentStore.get(Number(this.application.npoId), EntityTypeEnum.SupportingDocuments).subscribe(
      res => {
        this.documents = res.filter(x => x.entity === EntityEnum.FundedNpo);
      },
      (err) => {
  ...
  private loadDocumentTypes() {
    this._dropdownRepo.getEntities(DropdownTypeEnum.DocumentTypes, false).subscribe(
      (results) => {
        this.documentTypes = results.filter(x => x.location === DocumentUploadLocationsEnum.FundedNpo);
